...
```
Multiple left joins can cause severe system impacts.  This query timed out at over an hour to execute.  The report finally crossed four databases (3 oracle @ different versions, 1 SQL Server), > 20 source files, with > 30 view dependencies fishboning out to ~ 20 levels before the report output. 
```{sql}
from level_1 aa
